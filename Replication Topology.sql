...
* 
* Currently we support only SQL Server 2005 and later instances.
* *********************************************************************************
*/
...
BEGIN
	...
	SET @DISTRIBUTIONDBNAME = '???' 
	DECLARE @CURRENTDATABASE SYSNAME
	SELECT @CURRENTDATABASE = DB_NAME()
	SELECT @DISTRIBUTIONDBNAME = NAME FROM SYS.DATABASES (NOLOCK) WHERE IS_DISTRIBUTOR = 1 AND NAME = @CURRENTDATABASE
...
--					SET @CONSOLEMSG = ' |------- ' + CONVERT(VARCHAR(10), @NUMARTICLES) + ' article(s)'
			...
			BEGIN
				...
				DEALLOCATE SUBSCRIPTIONCURSOR
			END
			FETCH NEXT FROM PUBLICATIONCURSOR INTO @PUBLICATIONAME, @PUBLICATIONID, @PUBLICATIONTYPE, @PUBLICATIONDATABASE, @ALLOW_QUEUED_TRAN
		END
