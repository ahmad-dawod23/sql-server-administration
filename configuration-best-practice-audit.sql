...
-- Purpose : Review all key instance-level settings against
--           documented best practices. Flags deviations.
-- Safety  : All queries are read-only.
-- Note    : "Recommended" values are general-purpose starting points.
--           Adjust for your workload and environment.
-----------------------------------------------------------------------

-----------------------------------------------------------------------
-- 1. KEY sys.configurations BEST-PRACTICE REVIEW
-----------------------------------------------------------------------
SELECT
    c.[name]                                     AS Setting,
    ...
    CASE c.[name]
        ...
        -- Auto-close / auto-shrink (instance default — also check per-DB)
...
-----------------------------------------------------------------------
-- 2. DATABASE-LEVEL SETTINGS AUDIT
...
-----------------------------------------------------------------------
-- 3. INSTANT FILE INITIALIZATION CHECK
--    Reduces data file growth time dramatically.
...
-----------------------------------------------------------------------
-- 4. TEMPDB CONFIGURATION AUDIT
--    Best practice: multiple data files (1 per core, up to 8),
--    same initial size, same autogrowth, trace flag 1118 (pre-2016).
-----------------------------------------------------------------------
SELECT
    [name]                                        AS FileName,
...
-- Tempdb file count recommendation:
SELECT
...
-----------------------------------------------------------------------
-- 5. MAX MEMORY vs. PHYSICAL MEMORY CHECK
-----------------------------------------------------------------------
SELECT
...
-----------------------------------------------------------------------
-- 7. SQL SERVER VERSION & PATCH LEVEL
